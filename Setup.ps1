...
function Install-Office {
    $response = Prompt-YesNo -Message "Would you like to install Microsoft Office?" -Default "N"
    if ($response) {
        try {
...
function Restore-Activation {
    $response = Prompt-YesNo -Message "Would you like to check and restore Microsoft product activation?" -Default "N"
    ...
    if ($response) {
        try {
            ...
            if ($nonLicensedProducts) {
                ...
                if (Prompt-YesNo -Message "Would you like to invoke MAS to restore activation?" -Default "N") {
...
function Request-RestartExplorer {
    ...
    if (Prompt-YesNo -Message "Restart now?" -Default "N") {
        Restart-Computer
    }
}
